...
import { setDisplayDialog, setLeaderBoard, setName } from "../context/slice";
...
const Dialog = ({ onClose }) => {
  ...
  const handleSave = async (e) => {
    ...
    try {
      ...
      if (res.status === 200) {
        ...
        alert(
          "Es gab ein Problem beim Aktualisieren der Bestenliste. Bitte versuche es erneut."
        );
      }
    } catch (error) {
      console.error("Fehler beim Aktualisieren der Bestenliste:", error);
      alert(
        "Es gab einen Fehler beim Aktualisieren der Bestenliste. Überprüfe die Serververbindung."
      );
    }
  ...
  return (
    ...
    >
      ...
      >
        <label htmlFor="name-input">Bitte gib deinen Namen ein</label>
        <input
          id="name-input"
